...
这是kinar0自行维护自用的版本。增加了大量提示词生成约束以及角色特征约束、服装约束。七段式提示词，较好地适配anima的模型prompt习惯，争取最大限度实现角色特征、动作、服装控制
感谢原作者提供的danbooru skill。
...
- LLM 语义规划 + 本地 Danbooru 批量校验：角色、作品、服装来源、动作、道具等只在确认后成为 hard tags；角色变体服装会按帖子共现提取，不需要逐个硬编码。
- 图片法术解析。
- 视觉模型反推图片提示词。
...
每次生图可以单独指定尺寸，不会修改插件的默认设置。支持“方图/正方形”“竖图/竖版”“横图/横版”“长竖图/手机竖屏”“宽屏/超宽图”等说法，插件会从当前“可用尺寸列表”中选择比例最接近的一项。也可以写 `anm 1216x832 描述`（原有的 `1024x1536：描述` 仍兼容），或在描述末尾添加 `--尺寸 1216x832`；明确尺寸不在可用列表中时，插件会列出可选尺寸，不会静默替换。

自然语言生图默认由聊天模型自由发展统一主题，并丰富服装、姿态、材质、配饰、前景、构图、光影和少量特效。插件仍会保留固定角色身份、人数以及用户明确指定的关键服装、动作、表情和道具。需要完全按现成 tags 生成时，请使用 `/anm 无优化`。

修改：可以在提示词末尾输入#，然后自行补充tag。类似于`/anm 丰川祥子和千早爱音在梦中接吻#kiss, looking at another`#后面的内容会原样追加到tag中
对于铸币模型不认识的角色或服装套组（羽丘校服之类的），有一套已验证的danbooru tag检索和学习系统，尽可能保证模型认识新角色的外貌和服装
