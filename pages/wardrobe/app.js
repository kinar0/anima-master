...
const META = {
  ...
  outfitSets: { kicker: "NAMED OUTFIT SETS", title: "服装套组", description: "完整且独立命名的服装 anchor。命中后不会让 LLM 擅自补充套组组成。" },
...
function updateDirty() { elements.save.disabled = state.busy || !state.dirty; elements.dirtyBar.hidden = !state.dirty; elements.dataStatus.textContent = state.dirty ? "待保存" : "已同步"; }
...
function outfitEntry(item, index) {
  ...
  const select = node("select"); [["default","默认服装"],["stage","演出服"]].forEach(([value,label]) => { const option = node("option", "", label); option.value = value; option.selected = item.qualifier === value; select.append(option); });
