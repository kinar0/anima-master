...
现在多个角色也推荐直接写 `/anm <多个角色的描述>`，走普通结构化 prompt 路径并按角色绑定身份、外貌和服装。例如：

```text
/anm 丰川祥子穿默认服装坐在左边，千早爱音穿羽丘冬季校服站在右边，若叶睦穿黑色葬礼服装站在两人身后
```

`/anm 多人 <描述>` 仍保留为不常用的兼容入口，用于旧的 2–4 人专用规划、多人候选和强制视觉检查流程。

以下示例和专用构图、候选、自检说明只适用于这个兼容入口：
